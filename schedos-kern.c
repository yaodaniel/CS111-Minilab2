...
schedule(void)
{
	...
	if (scheduling_algorithm == 0) //Simple Round-Robin Scheduling
		while (1) {
			pid = (pid + 1) % NPROCS;
	...
	else if (scheduling_algorithm == 2) { //Exercise 4A (Priority scheduling)
		int i;
		while(1) {
			pid_t highestPriority = 0;
			pid = (pid + 1) % NPROCS;
	...
	else if (scheduling_algorithm == 3) { //Exercise 4B (Time Share scheduling)
		while(1) {
			if(proc_array[pid].p_state == P_RUNNABLE) {
